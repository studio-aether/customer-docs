---
title: Developer API
description: Server and client exports of aether_wings_demon, and how to drive it from an inventory item.
---
...
```lua
exports.vorp_inventory:registerUsableItem('demon_wings', function(data)
    if not data or not data.source then return end
    exports.aether_wings_demon:Toggle(data.source)
...
```lua
RSGCore.Functions.CreateUseableItem('demon_wings', function(src)
    exports.aether_wings_demon:Toggle(src)
end)
```
...
```lua
exports.aether_wings_demon:SetTint(src, 16, 'wings')
```
...
| Table | Holds |
|---|---|
| `aether_wings_demon_loadout` | one saved look per character |
| `aether_wings_demon_loadout_access` | the access list an admin manages in the menu |
